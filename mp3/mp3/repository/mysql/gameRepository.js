    ...
    return checkNameUnique(game.name)
        .then((err) => {
            if (err) {
                ...
                .then(res => {
                    console.log(res);
                    db.promise().query('INSERT INTO HISTORY (user, game, operation, space, email) VALUES (?, ?, ?, ?, ?)', [``, `${name}`, historyData.CREATE, 'game', '']);
                    return res;
                })
            }
        })
    ...
    return checkNameUnique(game.name, id)
        .then((err) => {
            if (err) {
                ...
                .then(res => {
                    console.log(res);
                    db.promise().query('INSERT INTO HISTORY (user, game, operation, space, email) VALUES (?, ?, ?, ?, ?)', [``, `${name}`, historyData.UPDATE, 'game', '']);
                    return res;
                })
            }
        })
...
exports.deleteGame = (id) => {
    ...
    let game;
    return db.promise().query('SELECT * FROM game WHERE id = ?', [id])
    .then(r => {
        game = r[0][0];
        ...
        return db.promise().query(sql1, [id])
            .then(() => {
                ...
                .then(res => {
                    console.log(res);
                    db.promise().query('INSERT INTO HISTORY (user, game, operation, space, email) VALUES (?, ?, ?, ?, ?)', [``, `${game.name}`, historyData.DELETE, 'game', '']);
                    return res;
                })
            })
    })
